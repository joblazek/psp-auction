...
# Sorting function based on utility maximization
def utility_based_sort(buyer, connected_sellers, G_seller_buyer, gamma=1.0):
...
# Function for a node to calculate damping factor based on influence sets
def influence_function(node_id, G_seller_buyer, G_buyer_buyer, G_seller_seller, influence_threshold, window_size=5):
    """
    Determines how a node (buyer or seller) influences the market.
    ...
    :param influence_threshold: Threshold
    :param window_size: Time window size for calculating moving average influence
    :return: damping factor alpha (0=no influence, 1=influence equal to self)
    """
    # Get the node object
...
def update_bids_psp_proportional_with_influence(G_seller_buyer, G_buyer_buyer, G_seller_seller, buyer_config, seller_config, influence_threshold):
    ...
    for node_id in G_seller_buyer.nodes:
        ...
        if influence_function(node_id, G_seller_buyer, G_buyer_buyer, G_seller_seller, influence_threshold):
...
def main_network_simulation_psp_proportional_with_influence():
    config = {
    ...
    for iteration in range(iterations):
        ...
        update_bids_psp_proportional_with_influence(G_seller_buyer, G_buyer_buyer, G_seller_seller, buyer_config, seller_config, config['influence_threshold'])
...
main_network_simulation_psp_proportional_with_influence()
